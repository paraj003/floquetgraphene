...
w=(2*pi);
T=2*pi/w;
tnn=0.9%1/besselj(0,1.43406956); %nearest neighbour coupling
tnnn=0.25*tnn; %next nearest neighbour coupling, next nearest neighbor coupling shifts dirac point in energy by 3t
Tdiv=100;
dt=T/Tdiv;
Lx=20% even
Ly=20;%even
...
rng(seedvalue);
Vrandarr=[0.0];
tnn_disarr=0.25*Vrandarr;
tnnn_disarr=0.25*0.25*Vrandarr;
...
%
% %%%%%%figure to check correct indices
%lattice points
% figure()
% plot(basvec(:,1),basvec(:,2),'o')
% hold on
% %nn
% for pp=1:length(nnindices)
% plot([basvec(nnindices(pp,1),1),basvec(nnindices(pp,2),1)],[basvec(nnindices(pp,1),2),basvec(nnindices(pp,2),2)],'-b')
% end
% %PBC nn
% for pp=1:length(PBCxnnindices)
% plot([basvec(PBCxnnindices(pp,1),1),basvec(PBCxnnindices(pp,2),1)],[basvec(PBCxnnindices(pp,1),2),basvec(PBCxnnindices(pp,2),2)],'-g')
% end
% for pp=1:length(PBCynnindices)
% plot([basvec(PBCynnindices(pp,1),1),basvec(PBCynnindices(pp,2),1)],[basvec(PBCynnindices(pp,1),2),basvec(PBCynnindices(pp,2),2)],'-g')
% end
% %nnn
% for pp=1:length(nnnindices)
% plot([basvec(nnnindices(pp,1),1),basvec(nnnindices(pp,2),1)],[basvec(nnnindices(pp,1),2),basvec(nnnindices(pp,2),2)],'-r')
% end
% %PBC nnn
% for pp=1:length(PBCxnnnindices)
% plot([basvec(PBCxnnnindices(pp,1),1),basvec(PBCxnnnindices(pp,2),1)],[basvec(PBCxnnnindices(pp,1),2),basvec(PBCxnnnindices(pp,2),2)],'-k')
% end
% for pp=1:length(PBCynnnindices)
% plot([basvec(PBCynnnindices(pp,1),1),basvec(PBCynnnindices(pp,2),1)],[basvec(PBCynnnindices(pp,1),2),basvec(PBCynnnindices(pp,2),2)],'-k')
% end
% for pp=1:length(PBCxynnnindices)
% plot([basvec(PBCxynnnindices(pp,1),1),basvec(PBCxynnnindices(pp,2),1)],[basvec(PBCxynnnindices(pp,1),2),basvec(PBCxynnnindices(pp,2),2)],'-k')
% end
