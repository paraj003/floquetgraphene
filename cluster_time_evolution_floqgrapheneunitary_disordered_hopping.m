...
%for time evolution plots use timeevo_plot
...
for Mchoice=1:length(Marr)
    ...
    for Vrandchoice=1:length(Vrandarr)
        ...
        En=zeros(Lx*Ly,disavmax);
        eigenvecs=zeros(Lx*Ly,Lx*Ly,disavmax);
        ...
        Name=sprintf('data/time-evo_ED_graphenefloquetdisorderdata%s-%s.mat',datestring,JobID);
        save(Name,'Lx','Ly','PBCx','PBCy','A','M','w','tnn','tnnn','T','Tdiv','dt','Vrand','disavmax','seedvalue','En','eigenvecs')
    end
